...
serialized_model_file = open(
    os.path.join("./examples/Scenario2/Example1", "serialized_model.obj"), "rb"
)
...
with torch.no_grad():
    for data, target in test_loader:
        print("Sample:", c)
        encrypted_outputs += [
            wrapped_model(data.view(-1, 28 * 28).flatten()).serialize()
        ]
...
filehandler = open(
    os.path.join("./examples/Scenario2/Example1", "encrypted_outputs.obj"), "wb"
)
