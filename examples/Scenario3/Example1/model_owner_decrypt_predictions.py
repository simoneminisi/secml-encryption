...
file_encrypted_output = open(os.path.join('./examples/Scenario3/Example1', 'encrypted_outputs.obj'), 'rb') 
encrypted_outputs = pickle.load(file_encrypted_output)

file_target = open(os.path.join('./examples/Scenario3/Example1', 'targets.obj'), 'rb') 
targets = pickle.load(file_target)

correct = 0
total = 0

context = ts.context_from(read_data(os.path.join('./examples/Scenario3/Example1', 'secret.txt')))
...
print(f'Test Accuracy of the model on the {total} test images with TenSEAL: {100 * correct / total:.2f}%')